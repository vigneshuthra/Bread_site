      ...
      <div data-aos="zoom-in" data-aos-delay="200" className='w-full grid lg:grid-cols-4 grid-cols-1 justify-center items-center gap-6'>
        ...
        {ourbreads.map((item, index) => (
          <div key={index} className=' flex flex-col justify-center items-center gap-4 bg-white p-8 rounded-lg'>
            ...
            <Image src={item.image} alt={''} width={300} height={400} className='w-[300px] h-[200px] rounded-lg transform hover:scale-110 transition-transform duration-300 cursor-pointer' />
            <h1 className='text-[20px] text-black font-semibold'>{item.title}</h1>
            <p className='text-center'>{item.desc}</p>
            <button className='border-2 border-black rounded-full px-8 py-2 uppercase text-sm font-semibold transform hover:scale-110 transition-transform duration-300 cursor-pointer'> Order NOW</button>
          </div>
        ))}
      </div>
